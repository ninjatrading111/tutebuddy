...
class PaymentController extends Controller
{
    ...
    public function getTransactions()
    {
        if(auth()->user()->hasRole('Instructor')) {
            ...
            $transactions  = Transaction::whereIn('order_id', $order_ids)->orWhere('user_id', auth()->user()->id)->paginate(50);
    ...
    public function orderDetail($id)
    {
        if(auth()->user()->hasRole('user')) {
            ...
            foreach($order->items as $item) {
                ...
                $schedule_ids = Schedule::whereIn('lesson_id', $lesson_ids)->pluck('id');
                if($schedule_ids->count() > 0) {
                    $refund_available = false;
                }
            }
            return view('backend.payment.student.order-detail', compact('order', 'refund_available'));
        } else {
